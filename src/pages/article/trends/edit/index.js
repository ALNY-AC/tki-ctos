export default {
    ...
    methods: {
        ...
        async update() {
            if (!this.isAdd) {
                const res = await this.$http.post('/article/info', { id: this.$route.query.id });
                if (res.code == 1) {
                    this.form = res.data;
                }
            } else {
        ...
        async submit() {
            ...
            try {
                const res = await this.$http.post('/article/save', this.form);
                if (res.code >= 0) {
                    if (this.isAdd) {
                    ...
                    this.$router.replace('/article/trends/list');
                }
            } catch (error) {
                return;
            }
